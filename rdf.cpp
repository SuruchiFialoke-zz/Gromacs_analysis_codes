...
#include <ctime>
#include <malloc.h>
#include </opt/gromacs/4.5.5/include/xdrfile/xdrfile_xtc.h> // xdr include file 
...
cin>>firstsnap ;
        ...
        //int read_xtc(XDRFILE*, int, int*, float*, float (*)[3], float (*)[3], float*)
        ...
        //if(step%50000==0) {
        //cout<<step<<"\t"<<xtc_coor[natoms-1][0]<<"\t"<<xtc_coor[natoms-1][1]<<"\t"<<xtc_coor[natoms-1][2]<<endl;
        //      }
        ...
        for(int k=0;k<n_bins;k++)
        {
                ...
                //g_r[k] /= 4.0*M_PI/3.0*(pow((k+1),3)-pow(k,3))*pow(dr,3)*rho_bulk;
